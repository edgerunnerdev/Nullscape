...
void InputManager::HandleMouseButtonEvent(const SDL_MouseButtonEvent& event, ButtonState state)
{
    for (auto& callbackInfo : m_MouseCallbacks)
    {
        ...
        else if ((event.button == SDL_BUTTON_LEFT && callbackInfo.button == MouseButton::Left) || (event.button == SDL_BUTTON_RIGHT && callbackInfo.button == MouseButton::Right) ||
                 (event.button == SDL_BUTTON_MIDDLE && callbackInfo.button == MouseButton::Middle))
        {
            callbackInfo.callback();
        }
    }
...
{
    m_KeyboardCallbacks.remove_if([token](const InputCallbackKeyboardInfo& callbackInfo) {
        return token == callbackInfo.token;
    });
}

void InputManager::RemoveMouseCallback(InputCallbackToken token)
{
    m_MouseCallbacks.remove_if([token](const InputCallbackMouseInfo& callbackInfo) {
        return token == callbackInfo.token;
    });
}
...
{
    m_MouseWheelCallbacks.remove_if([token](const InputCallbackMouseWheelInfo& callbackInfo) {
        return token == callbackInfo.token;
    });
}
} // namespace Genesis