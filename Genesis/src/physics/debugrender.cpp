...
{
    FrameWork::GetDebugRender()->DrawLine(glm::vec3(from.x(), from.y(), from.z()), glm::vec3(to.x(), to.y(), to.z()), glm::vec3(color.x(), color.y(), color.z()));
}

void DebugRender::drawSphere(const btVector3& p, btScalar radius, const btVector3& color)
{
    FrameWork::GetDebugRender()->DrawCircle(glm::vec3(p.x(), p.y(), p.z()), radius, glm::vec3(color.x(), color.y(), color.z()));
}
