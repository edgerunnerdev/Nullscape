...
::FMOD::Sound* SoundManager::CreateSound(ResourceSound* pResourceSound)
{
    ...
    if (pResourceSound->IsPlaylist() == false)
    {
        ...
        {
            FrameWork::GetLogger()->LogWarning("SoundManager::CreateSound ('%s'): Unable to create sound, resource not set as streaming or effect.",
                                               pResourceSound->GetFilename().GetFullPath().c_str());
            return nullptr;
        }
...
void SoundManager::SetListener(const glm::vec3& position, const glm::vec3& velocity, const glm::vec3& forward, const glm::vec3& up)
{
    ...
    m_pSystem->set3DListenerAttributes(0, &fmodPosition, &fmodVelocity, 0, 0);
}
