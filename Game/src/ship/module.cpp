...
{
    if ( pDealtBy != nullptr && 
         pDealtBy->HasPerk( Perk::Disruption ) && 
...
void Module::PlayDeathSFX()
{
	if ( m_pDeathSFX != nullptr )
	{
		...
		if ( pSoundInstance != nullptr )
		{
			...
			pSoundInstance->Set3DAttributes( &worldTranslation );
...
void EngineModule::Update( float delta )
{
	...
	{
		const bool tryingToMove = 
			m_pOwner->GetThrust() != ShipThrust::None || 
			m_pOwner->GetSteer() != ShipSteer::None ||
			m_pOwner->GetStrafe() != ShipStrafe::None;
...
m_RechargeRate( 0.0f )
{
	...
	m_Capacity = pReactorInfo->GetCapacity();
	m_RechargeRate = pReactorInfo->GetRechargeRate();
}

float ReactorModule::GetCapacity() const 
{ 
	return IsEMPed() ? 0.0f : m_Capacity;
}

float ReactorModule::GetRechargeRate() const 
{ 
