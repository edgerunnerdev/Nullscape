			...
			if ( ImGui::CollapsingHeader( ss.str().data() ) )
			{
				...
				for ( int idx = 0; idx < static_cast<int>( FactionId::Count ); ++idx )
				{
					...
					if ( damage > 0.0f )
					{
						...
						if ( ( factionId == FactionId::Player && damage > highestContributorDamage * ShipRewardThreshold ) ||
							 ( factionId == highestContributor ) )
						{
							overrideColour = true;
						...
						ImGui::Columns(2);
						ImGui::Text( "%s", g_pGame->GetFaction( factionId )->GetName().data() ); ImGui::NextColumn();
						ImGui::Text( "%d", static_cast<int>( damage ) ); ImGui::NextColumn();
						ImGui::Columns(1);
