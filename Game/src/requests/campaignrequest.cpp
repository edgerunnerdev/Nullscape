...
void CampaignRequest::UpdateExpansionArc( float delta )
{
	...
	if ( m_pExpansionGoal == nullptr && 
		 pBlackboard->Exists( sFirstExpansionEvent ) && 
		 pBlackboard->Exists( sFirstExpansionEventCompleted ) == false )
	{
	...
	if ( pBlackboard->Exists( sFirstExpansionEventCompleted ) == false && 
		 pSectorAster->GetFaction()->GetFactionId() == FactionId::Empire )
	{
		g_pGame->AddIntel( GameCharacter::FleetIntelligence, 
	...
	if ( pBlackboard->Exists( sFirstExpansionEventCompleted ) && 
		 pBlackboard->Exists( sSecondExpansionEvent ) == false &&
		 pSectorSyrion->GetFaction()->GetFactionId() != FactionId::Empire )
	{
		g_pGame->AddIntel( GameCharacter::FleetIntelligence,
	...
	if ( m_pExpansionGoal == nullptr && 
		 pBlackboard->Exists( sSecondExpansionEvent ) && 
		 pBlackboard->Exists( sSecondExpansionEventCompleted ) == false )
	{
...
void CampaignRequest::UpdatePirateArc( float delta )
{
	...
	else if ( pBlackboard->Exists( sFirstPirateEventCompleted ) == false && 
		      pBlackboard->Exists( sKillPirateFlagshipCompleted ) == false )
	{
	...
	else if ( pBlackboard->Exists( sFirstPirateEventCompleted ) && 
			  pBlackboard->Exists( sKillPirateFlagship ) == false &&
			  pBlackboard->Exists( sKillPirateFlagshipCompleted ) == false )
	{
		g_pGame->AddIntel( GameCharacter::FleetIntelligence,
	...
	else if ( pBlackboard->Exists( sKillPirateFlagship ) &&
		      pBlackboard->Exists( sKillPirateFlagshipCompleted ) == false )
	{
...
void CampaignRequest::UpdateMarauderArc( float delta )
{
	...
	if ( pBlackboard->Exists( sConquerMuspell ) && pBlackboard->Exists( sConquerMuspellCompleted ) == false && GoalExists( pSectorMuspell ) == false )
	{
        ...
        SetThreatScore( 
            GetShipThreatScore( "special_battlestation", FactionId::Marauders ) +
            GetShipThreatScore( "special_naglfar", FactionId::Marauders ) );
	}
	...
	if ( pBlackboard->Exists( sConquerSurtrCompleted ) && 
		 pBlackboard->Exists( sConquerMuspellCompleted ) && 
		 pBlackboard->Exists( invasionTag ) == false )
	{
		g_pGame->AddIntel( GameCharacter::FleetIntelligence,
	...
	else if ( pBlackboard->Exists( sConquerValhalla ) == false &&
		 pBlackboard->Exists( invasionTag ) &&
		 pBlackboard->Get( invasionTag ) == 0 )
	{
		pSectorValhalla->SetHyperspaceInhibitor( false );
		pBlackboard->Add( sConquerValhalla );
	}
	else if ( pBlackboard->Exists( sConquerValhalla ) &&
		pBlackboard->Exists( sConquerValhallaCompleted ) == false &&
		GoalExists( pSectorValhalla ) == false )
	{
		AddGoal( std::make_shared< RequestGoal >( pSectorValhalla, "Campaign: capture Valhalla", sCampaignGoalColour ) );
	}
	else if ( pBlackboard->Exists( sConquerValhalla ) && 
		pSectorValhalla->GetFaction()->GetFactionId() == FactionId::Empire )
	{
