...
bool FleetBehaviour::ProcessTurn()
{
    ...
    if (pSectorToAssist != nullptr)
    {
        ...
        Genesis::Core::Log::Info() << "[" << m_pFleet->GetFaction()->GetName() << "] Fleet diverted to assist sector [" << x << "," << y << "]." << GetAssistingFleetsCount(pSectorToAssist)
                                   << " fleets now assisting the sector.";
#endif
...
{
    if (pSectorInfo->HasHyperspaceInhibitor() && pSectorInfo->GetFaction() != m_pFleet->GetFaction() && pSectorInfo->GetContestedFleets().size() < cMaxContestingFleets &&
        pSectorInfo->IsPersonal() == false)
    {
        return false;
    }
