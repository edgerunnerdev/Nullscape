...
bool FleetBehaviour::ProcessTurn()
{
	...
	else if ( m_pFleet->HasArrived() )
	{
		...
		else if ( pCurrentSector->HasHyperspaceInhibitor() == false && 
			pCurrentSector->IsPersonal() == false &&
			pCurrentSector->GetContestedFleets().size() < cMaxContestingFleets )
		{
...
bool FleetBehaviour::CanAttackSector( SectorInfo* pSectorInfo ) const
{
	if ( pSectorInfo->HasHyperspaceInhibitor() && 
		 pSectorInfo->GetFaction() != m_pFleet->GetFaction() && 
		 pSectorInfo->GetContestedFleets().size() < cMaxContestingFleets &&
