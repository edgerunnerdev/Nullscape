...
void NewGameWindow::Update()
{
	...
	else if ( m_CurrentPage == PageId::FactionPresence )
	{
		const bool canCreate = 
			( m_GalaxyCreationInfo.GetFactionPresence( FactionId::Pirate ) != FactionPresence::None ) ||
			( m_GalaxyCreationInfo.GetFactionPresence( FactionId::Marauders ) != FactionPresence::None ) ||
			( m_GalaxyCreationInfo.GetFactionPresence( FactionId::Ascent ) != FactionPresence::None ) ||
			( m_GalaxyCreationInfo.GetFactionPresence( FactionId::Iriani ) != FactionPresence::None ) ||
			( m_GalaxyCreationInfo.GetFactionPresence( FactionId::Hegemon ) != FactionPresence::None );
