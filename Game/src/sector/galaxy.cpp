...
void Galaxy::EndGameCheck()
{
	...
	{
		if ( pBlackboard->Exists( sGameEnd ) == false &&
			 pBlackboard->Exists( sExpansionArcFinished ) &&
			 pBlackboard->Exists( sPirateArcFinished ) &&
			 pBlackboard->Exists( sMarauderArcFinished ) &&
			 pBlackboard->Exists( sAscentArcFinished ) &&
			 pBlackboard->Exists( sChrysamereArcFinished ) && 
			 pBlackboard->Exists( sIrianiArcFinished ) )
		{
	...
	{
		if ( pBlackboard->Exists( sGameEnd ) == false &&
			 pBlackboard->Exists( "#infinity_war_finished" ) )
		{
			m_pRep->GetGalaxyWindow()->ShowEndGameWindow();
			pBlackboard->Add( sGameEnd );
		}
	...
	if ( m_DebugWindowOpen )
	{
		...
		for ( int i = 0; i < (int)FactionId::Count; ++i ) 
		{
			...
			if ( ImGui::CollapsingHeader( pFaction->GetName().c_str() ) )
			{
				...
				ImGui::Text( "Collapsing" ); ImGui::NextColumn();
				ImGui::Text( "%s", pFaction->IsCollapsing() ? "True" : "False" ); ImGui::NextColumn();

				ImGui::Text( "Sectors" ); ImGui::NextColumn();
				ImGui::Text( "%zu", pFaction->GetControlledSectors().size() ); ImGui::NextColumn();

				ImGui::Text( "Fleets" ); ImGui::NextColumn();
				ImGui::Text( "%zu", pFaction->GetFleets().size() ); ImGui::NextColumn();

				ImGui::Text( "Fleets in construction" ); ImGui::NextColumn();
				ImGui::Text( "%zu", pFaction->GetFleetsInConstruction().size() ); ImGui::NextColumn();
				...
				ImGui::Text( "Shipyards" ); ImGui::NextColumn();
