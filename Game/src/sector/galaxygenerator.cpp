...
{
	if ( presence == FactionPresence::None ) return 0;
	else if ( presence == FactionPresence::Light ) return 3 + rand() % 2;
	else if ( presence == FactionPresence::Standard ) return 6 + rand() % 3;
	else if ( presence == FactionPresence::Heavy ) return 10 + rand() % 4;
	else return 0;
}
		...
		for ( Data& data : m_Data )
		{
			...
			if ( generationType == GalaxyCreationInfo::GenerationType::Expansion )
			{
				...
				for ( SectorInfo* pControlledSector : controlledSectors )
				{
					...
					{
						if ( pBorderingSector->GetFaction()->GetFactionId() == FactionId::Neutral && 
							 std::find( controlledSectors.cbegin(), controlledSectors.cend(), pBorderingSector ) == controlledSectors.cend() )
						{
