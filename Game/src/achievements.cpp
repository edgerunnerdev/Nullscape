...
void AchievementsManager::UnlockAchievement( AchievementID achievementID )
{
	...
	// all stats would get aggregated, but Hyperscape doesn't have stat-based
...
void AchievementsManager::UpdateDebugUI()
{
	if ( Genesis::ImGuiImpl::IsEnabled() && m_DebugUIOpen )
	{
		...
		ImGui::Text( "Unlocked" ); ImGui::NextColumn();
		ImGui::Separator();

		for ( const Achievement_t& achievement : g_Achievements )
		{
			ImGui::Text( achievement.idAsCString ); ImGui::NextColumn();
			ImGui::Text( achievement.name ); ImGui::NextColumn();
