...
bool Shipyard::CanBeUsed() const
{
	...
	return ( pPlayerShip != nullptr && 
		 m_pDockedShip == nullptr && 
		 pCurrentSector->IsPlayerVictorious() &&
		 glm::distance( pPlayerShip->GetTowerPosition(), GetPosition() ) < GetDockingRange() &&
		 pPlayerShip->GetHyperspaceCore()->IsCharging() == false &&
		 pPlayerShip->GetHyperspaceCore()->IsJumping() == false );
}
...
bool Shipyard::ValidateDockedShip()
{
	...
	else
	{
			...
			for ( int y = y1; y <= y2; ++y )
			{
				...
				if ( pModule != nullptr )
				{
					...
					else if ( type == ModuleType::Reactor )
					{
						...
						energyGenerated += pReactorInfo->GetRechargeRate();
					}
					else if ( type == ModuleType::Shield )
					{
						ShieldInfo* pShieldInfo = static_cast<ShieldInfo*>( pModule->GetModuleInfo() );
                        energyUsed += pShieldInfo->GetEnergyUsage( m_pDockedShip );
					}
		...
		else if ( energyGenerated < energyUsed )
		{
			g_pGame->AddFleetCommandIntel( "The shield generators consume more energy than the reactors can handle, Captain." );
			return false;
		}
		else if ( numAddons > 6 )
		{
...
void Shipyard::LoadFromFile( const std::string& filename )
{
	...
	if ( m_pDockedShip->GetModuleHexGrid().GetUsedSlots() > 0 )
	{
		for ( int x = x1; x <= x2; ++x )
		{
			for ( int y = y1; y <= y2; ++y )
			{
				m_pDockedShip->RemoveModule( x, y );
			}
		}
	}
